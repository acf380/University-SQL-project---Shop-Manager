...
{	
	...
	switch(buttonName) {
		...
		case 'Save!' : 
			var jsonString = '{"kategorie":[';

            var catId = document.getElementById('tab1').getElementsByTagName('tbody')[0].getElementsByTagName('tr')[n].getElementsByTagName('td')[0].getElementsByTagName('label')[0];
			jsonString += `{"val":"${catId.innerText}"},`;
...
{											
	...
	var hiddeninput = document.createElement("input");
	hiddeninput.setAttribute("type", "hidden");
	hiddeninput.setAttribute("name", "kategoria");
